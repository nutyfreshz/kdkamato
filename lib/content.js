...
function byNewest(a, b) {
  ...
  const idA = Number(String(a.Episode_ID || a.Article_ID || '').replace(/\D/g, '')) || 0;
  const idB = Number(String(b.Episode_ID || b.Article_ID || '').replace(/\D/g, '')) || 0;
  return idB - idA;
}
...
function hydrateArticle(row) {
  const coverId = extractDriveId(row.Cover_Asset);
  return {
    ...row,
    Featured: normalizeBoolean(row.Featured),
    cover: coverId
      ? {
          id: coverId,
          url: `/api/drive-image?id=${encodeURIComponent(coverId)}`
        }
      : null
  };
}

export async function getMangaIndex({ limit = 500, status = LIVE_STATUS } = {}) {
...
export async function getManga({ limit = 12, status = LIVE_STATUS } = {}) {
  const selected = await getMangaIndex({ limit, status });
