export const LAB_RULESETS = {
  C1: { id: 'LAB_RULESET_C1_v1.0', version: '1.0', evidenceClass: 'DIRECT + GEOMETRY/TENDENCY', lastReviewed: '2026-09-05' },
  C2: { id: 'LAB_RULESET_C2_v1.0', version: '1.0', evidenceClass: 'GEOMETRY + EVIDENCE-SUPPORTED TENDENCY', lastReviewed: '2026-09-05' },
  C3: { id: 'LAB_RULESET_C3_v1.0', version: '1.0', evidenceClass: 'DIRECT + SCENARIO MATH', lastReviewed: '2026-09-05' },
  Q1: { id: 'LAB_RULESET_Q1_v1.0', version: '1.0', evidenceClass: 'DERIVED MEASUREMENT', lastReviewed: '2026-09-05' },
  Q2: { id: 'LAB_RULESET_Q2_v1.0', version: '1.0', evidenceClass: 'DERIVED MEASUREMENT', lastReviewed: '2026-09-05' },
  Q3: { id: 'LAB_RULESET_Q3_v1.0', version: '1.0', evidenceClass: 'DIRECT + DERIVED MARKERS', lastReviewed: '2026-09-05' },
  Q4: { id: 'LAB_RULESET_Q4_v1.0', version: '1.0', evidenceClass: 'DERIVED MEASUREMENT', lastReviewed: '2026-09-05' },
  Q5: { id: 'LAB_RULESET_Q5_v1.0', version: '1.0', evidenceClass: 'DERIVED MEASUREMENT', lastReviewed: '2026-09-05' }
};
...
export function computeFrame(height, wrist, ankle) {
  const h = num(height); const w = num(wrist); const a = num(ankle);
  if (!h || !w || !a) return null;
  return { heightWrist: round(h / w, 2), heightAnkle: round(h / a, 2) };
}
...
export function exerciseFitResult({ height, armSpan, femur, tibia, torso, movement }) {
  ...
  const reachText = ape.diff === 0 ? 'Arm span ใกล้เคียงกับส่วนสูง' : ape.diff > 0 ? `Arm span ยาวกว่าส่วนสูง ${ape.diff} cm` : `Arm span สั้นกว่าส่วนสูง ${Math.abs(ape.diff)} cm`;

  if (movement === 'BENCH') return {
    code: ape.diff >= 0 ? 'C1_BENCH_REACH_NONNEGATIVE' : 'C1_BENCH_REACH_NEGATIVE',
    result: `${reachText} · Ape ${ape.ratio}×`,
    meaning: `${reachText}. ถ้า setup อื่นเท่ากัน reach ที่ยาวขึ้นหรือลดลงสามารถเปลี่ยนระยะที่ bar ต้องเดินทางใน pressing ได้`,
    ...
    nextHref: '/lab/ape-index', nextLabel: 'OPEN APE INDEX'
  };

  if (movement === 'DEADLIFT') return {
    code: 'C1_DEADLIFT_CONSERVATIVE_GEOMETRY',
    result: `${reachText} · Femur:Tibia ${ft.ratio}`,
    ...
    nextHref: '/lab/ape-index', nextLabel: 'SEE REACH CONTEXT'
  };

  return {
    code: ft.percent >= 0 ? 'C1_SQUAT_FEMUR_RELATIVE_LONGER' : 'C1_SQUAT_FEMUR_RELATIVE_SHORTER',
    result: `Femur:Tibia ${ft.ratio} · Torso proxy ${tr} cm`,
    ...
    nextHref: '/lab/squat-geometry', nextLabel: 'OPEN SQUAT GEOMETRY'
  };
}
...
export function squatScenarioCopy({ heel, stance, variant, kneeTravel }) {
  ...
  const variantCopy = variant === 'front'
    ? 'Front Squat โดยทั่วไปต้องจัด torso/bar relationship ต่างจาก Back Squat และมัก demanding ต่อการรักษาตำแหน่งลำตัวตั้งมากกว่า'
    : variant === 'lowbar'
      ? 'Low-Bar Back Squat ยอมให้ใช้ forward trunk lean และ hip contribution ได้มากขึ้นในหลาย setup'
      : 'High-Bar Back Squat เป็นจุดเทียบที่อยู่ระหว่าง Front และ Low-Bar ในหลาย pattern';
