...
export function ToolHeader({ id, title, question, role = 'LAB TOOL' }) {
  return <><LabStyles/><header className="lab-tool-head"><p className="eyebrow cyan">KDKAMATO LAB / {role}</p><p className="meta">{id}</p><h1>{title}</h1><p className="lab-tool-question">{question}</p></header></>;
}

export function MeasurementField({ label, unit, value, onChange, step = '0.1', guide, mistake, type = 'length' }) {
  ...
  return <div className="measure-field">
    ...
    <button type="button" className="measure-help" onClick={() => setOpen(v => !v)}>{open ? 'HIDE GUIDE' : 'HOW TO MEASURE'}</button>
...
export function ResultContract({ result, meaning, use, watch, nextHref, nextLabel, resultCode, children }) {
  ...
  return <div className="result-contract" aria-live="polite">
    <section><span>YOUR RESULT</span><h2>{result}</h2>{resultCode && <small>RESULT CODE: {resultCode}</small>}</section>
...
export function ShareResult({ title, text }) {
  ...
  async function share() {
    const payload = `${title}\n${text}\nKDKAMATO LAB`;
    try {
      if (navigator.share) await navigator.share({ title, text: payload });
      else if (navigator.clipboard) { await navigator.clipboard.writeText(payload); setStatus('COPIED'); setTimeout(() => setStatus('SHARE RESULT'), 1800); }
    } catch (_) {}
  }
